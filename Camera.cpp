...
Camera::Camera(const glm::vec3& pos, const glm::vec3& lookAt, const glm::vec3& up, int width, int height){
    this->pos = pos;
...
Ray Camera::generateRay(int i, int j, int pixWidth, int pixHeight){
    ...
    float u = l + ((r-l) * (i+0.5))/static_cast<float>(pixWidth);
    float v = b + ((t-b) * (j+0.5))/static_cast<float>(pixHeight);
